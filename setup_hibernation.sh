...
check_disk_encryption() {
    echo -e "\n${BLUE}=== Disk Encryption Check ===${NC}"
    
    local encrypted_detected=0
    
    # Check for LUKS encrypted devices
    if command -v cryptsetup >/dev/null 2>&1; then
        if cryptsetup status /dev/mapper/* 2>/dev/null | grep -q "LUKS"; then
            encrypted_detected=1
            print_status "ERROR" "LUKS encrypted devices detected"
        fi
    fi
    
    # Check for encrypted filesystems using lsblk
    if command -v lsblk >/dev/null 2>&1; then
        if lsblk -f | grep -q "crypt"; then
            encrypted_detected=1
            print_status "ERROR" "Encrypted filesystems detected"
        fi
    fi
    
    # Check for dm-crypt devices
    if [[ -d /dev/mapper ]]; then
        if ls /dev/mapper/ | grep -q -E "(crypt|luks|encrypted)"; then
            encrypted_detected=1
            print_status "ERROR" "DM-Crypt encrypted devices detected"
        fi
    fi
    ...
    if swapon --show 2>/dev/null | grep -q "/dev/mapper"; then
        encrypted_detected=1
        print_status "ERROR" "Encrypted swap detected"
    fi
    
    # If encryption is detected, exit the script
    if [[ $encrypted_detected -eq 1 ]]; then
        echo
        echo -e "${RED}=== HIBERNATION NOT AVAILABLE ===${NC}"
        print_status "ERROR" "This system uses disk encryption"
        print_status "ERROR" "Hibernation is not available on encrypted systems"
        print_status "ERROR" "The hibernation image would contain sensitive data in plain text"
        print_status "ERROR" "This poses a security risk and is not supported"
        echo
        echo "Alternatives to hibernation on encrypted systems:"
        echo "• Use suspend-to-RAM (sleep) instead"
        echo "• Implement secure hibernation with TPM/secure boot (advanced)"
        echo "• Use full disk encryption with secure hibernation setup"
        echo
        exit 1
    fi
    
    print_status "OK" "No disk encryption detected - hibernation is available"
}
...
check_filesystem() {
    ...
    # Check for encrypted filesystems (informational only now)
    if command -v lsblk >/dev/null 2>&1; then
        ...
        if [[ $encrypted_devices -gt 0 ]]; then
            ...
            print_status "INFO" "Note: Encryption check already performed above"
        fi
    fi
}
